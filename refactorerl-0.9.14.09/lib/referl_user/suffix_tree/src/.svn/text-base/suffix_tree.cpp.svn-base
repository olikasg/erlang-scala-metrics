...
/* Constant location definition of tokens file. */
...
CLONE* ST_CollectClones_new(SUFFIX_TREE* tree, int minlen, int minf, char*filename)
{      
    CLONE* head=NULL;
...
CLONE* clone_detection_by_suffix_tree(char *filename, long minlen, long minclones, long overlap_allowed)
{
    SUFFIX_TREE* tree;
    ...
    fclose(file);
    CLONE* head = NULL;
    head = ST_CollectClones_new(tree,minlen, minclones, filename);
    if(freestr == 1)
    free(str);
    ST_DeleteTree(tree);
    return head;
...
extern "C" {
    ...
    ERL_NIF_TERM get_clones(ErlNifEnv* env, struct clone* head)
    {
        struct clone* temp;
        RANGE* ranges;
        if (head == NULL){
        ...
        vector<ERL_NIF_TERM> clones;
        for (temp = head; temp != NULL; temp = temp->next)
        {
            vector<ERL_NIF_TERM> pairs;
            for (ranges = temp->ranges; ranges != 0; ranges = ranges->next) 
            {
                pairs.push_back(enif_make_tuple2(env, 
                                                 enif_make_int(env, ranges->start), 
                                                 enif_make_int(env, ranges->end)));
            }
            ERL_NIF_TERM apairs[pairs.size()];
            for (unsigned int i = 0; i < pairs.size(); i++)
            {
                apairs[i] = pairs[i];
            }
            clones.push_back(enif_make_tuple3(env, 
                                              enif_make_list_from_array(env, apairs, pairs.size()), 
                                              enif_make_int(env, temp->length), 
                                              enif_make_int(env, temp->freq)));
        }
        ERL_NIF_TERM aclones[clones.size()];
        for (unsigned int i = 0; i < clones.size(); i++) 
        {
            aclones[i] = clones[i];
        }
        ERL_NIF_TERM ret = enif_make_list_from_array(env, aclones, clones.size());
        return ret;
    };
    ...
    string erl_str2str(ErlNifEnv* env, const ERL_NIF_TERM& erl_str) {
        unsigned str_size;
        char* str_buff;
        string str;
    ...
    ERL_NIF_TERM search_dupcode(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[]) {
        string url = erl_str2str(env, argv[0]);
        char* c_url = new char[url.size()+1];
        copy(url.begin(), url.end(), c_url);
        c_url[url.size()] = '\0';
        
        int minlen = term2int(env, argv[1]);
        int minclones = term2int(env, argv[2]);
        int overlap = term2int(env, argv[3]);
        
        //clone_detection_by_suffix_tree(c_url,5,2,8);
        CLONE* head = NULL;
        head = clone_detection_by_suffix_tree(c_url,minlen,minclones,overlap);
        
        delete[] c_url;
        return get_clones(env, head);
    };
    
    static ErlNifFunc nif_funcs[] = {
            {"search_dupcode", 4, search_dupcode}
    };
    
    ERL_NIF_INIT(refusr_clone_identifierl_suffix_tree,nif_funcs,load,reload,upgrade,unload)
}
