...
state_to_disp(#state{action=selection, type=Type, res=Res,
    ...
    Disp = [{group_by, GroupbyType, Entity, list, Type, List}||
...
format(Dict, {group_by, Type, Entity, list, LType, LEntities}, scriptable) ->
    [{"", text(Type, Entity)++"\n"},
...
format(Dict, {group_by, Type, Entity, list, LType, LEntities}, formattednodes) ->
...
text0(expression, Expr) ->
    Text = string:strip(?Syn:flat_text2(Expr)),
...
text0(clause, Clause)->
    ...
    [Mod] = ?Query:exec(Fun, ?Fun:module()),
    Index  = refcore_syntax:index(Form, funcl, Clause),
