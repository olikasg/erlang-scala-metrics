...
% dets
-export([insert_dets/1, match_key_dets/1, match_name_dets/2, get_group_dets/3,
...
%position
-export([get_clones_by_pos/2,get_real_form/1, get_file/1]).
% formating
...
format(ClonesList, Options) when is_list(Options) ->           
    ...
    Result = ?MISC:parallelise(ClonesList, ?MODULE, format_worker, MakeProperArgs),
    {_, FormattedClones} = lists:unzip(Result),
    FormattedClones.

format_worker(ClonesList, Options)-> 
    UnzippedClones = lists:map(fun(Clone) -> unzip_clones(Clone) end, ClonesList),
    case proplists:get_value(format, Options) of
        prettyprint ->
            [try 
                Formatted = lists:map(fun(K) -> pretty_print(K) end, Clone),
                io_lib:format("~s", [Formatted])++
                delimiter2()
            catch
                _:_ -> ""
            end || Clone<-UnzippedClones, Clone /=[] ];
        file_and_loc -> 
            ExtClones = extend_file_and_loc_map(UnzippedClones, Options),
            remove_nodes(ExtClones);
        ui_format -> %old
            extend_file_and_loc_map(UnzippedClones, Options);
        nodes -> 
            lists:map(fun(K) ->
                lists:map(fun(L) ->
...
flat_text(Node = {_,Type, _})
    when Type =:= expr; Type =:= clause; Type =:= form->
    flat_text0(Node);
...
unzip_nth_unit(_, Act, Max, Res) when Act > Max -> Res;
unzip_nth_unit(Clone, Act, Max, Res) ->
    Units = unzip_nth_unit(Clone, Act, []),
    unzip_nth_unit(Clone, Act+1, Max, Res++[Units]).

unzip_nth_unit([], _, Res) -> Res;
unzip_nth_unit([Head | Tail], Act, Res) ->
    ActClone = lists:nth(Act, Head#clone_item.items),
    unzip_nth_unit(Tail, Act, Res++[ActClone]). 
...
extend_file_and_loc_map(Clones, Options) ->
    PosType = proplists:get_value(postype, Options),
    lists:map(fun(K) -> extend_file_and_loc(K, PosType) end, Clones).
...
validate_options(?STree, Options) when is_list(Options) ->
    MandatoryOptKeys = [minlen, minnum, overlap, files, name],
    OptionalOptKeys = [],
    lists:all(fun(Key)->
            ?TrueOrThrow(validate(Key, proplists:get_value(Key, Options, undef)),
                badarg(Key))
...
validate_options(?FSTree, Options) when is_list(Options) ->
    MandatoryOptKeys = [minlen, minnum, overlap, files, name, max_invalid_seq_length],
    OptionalOptKeys = [],
    lists:all(fun(Key)->
            ?TrueOrThrow(validate(Key, proplists:get_value(Key, Options, undef)),
                badarg(Key))
...
validate(algorithm, Algorithm)->
    lists:member(Algorithm, [sw_metrics, matrix, suffix_tree, filtered_suffix_tree, matrixfilter]);
validate(format, Format) ->
    lists:member(Format, [prettyprint, file_and_loc, file, nodes]);
...
validate(overlap, Overlap) ->
    is_integer(Overlap) andalso (Overlap > -1);
...
remove_nodes([]) -> [];
remove_nodes([Clone | Clones]) ->
        NoNodeClone = [proplists:delete(nodes, Cl) || Cl <- Clone],
        [NoNodeClone] ++ remove_nodes(Clones).
...
need_to_tlexpr(#clone_item{items = Items}) ->
     Bool = lists:any(fun(#unit{id = Id}) ->
                         ?STree:is_node_type([Id], [form,clause,func])
                     end, Items),
     Bool.
...
get_exprs(GroupClause) ->
    case ?STree:is_diff_length(GroupClause) of
...
match_name_dets(Name, Format)->
    ...
    case get_name_from_dets(Name) of
        {false, Reason} ->
            {false, Reason};
        {true, Result, Options} ->
            {true, format_results(Result, Options, Format)++[{options,Options}]}
    end.
...
create_key(Options) ->
    UnneccessaryOpts = [format, name, output, cache, enforce, positions],
...
get_algorithm_data(suffix_tree) ->
    ...
    DefOverlap = proplists:get_value(overlap, DefaultOptions),
    ...
    [{minlen, [{key, minlen}, {label, "Minlen"}, {default, DefMinlen}, {type, integer}]},
     ...
     {overlap, [{key, overlap}, {label, "Overlap"}, {default, DefOverlap}, {type, integer}]},
...
trim_sw_metrics(Start,End) ->
    {Ancestor,{{_,LeftChild},{_,RightChild}}} =
        ?STree:minimal_common_ancestor_with_children([Start,End]),
    Res = 
        case Ancestor of
            {file,Node} ->
...
%%% ----------------------------------------------------------------------------
%%% Functions to get the closes top level expression from the position
