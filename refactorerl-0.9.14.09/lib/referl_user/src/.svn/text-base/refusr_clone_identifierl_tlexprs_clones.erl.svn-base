...
-export([diags_worker/3, main_filter_worker/2]).
...
filter_out_clones(Clones, Options)->
     ...
     MakeProperArgs = fun(Arg)-> [Arg, {DiffLimit, MaxInvalidSeqLength}] end,
 ...
 main_filter_worker(Clones, Opts)->
    main_filter_worker(Clones, Opts, []).

 main_filter_worker([],_, Results) ->
     Results;
 main_filter_worker([Clone | Clones], Opts, Results) ->
     case filter_conditions(Clone, Opts) of
         false ->
             main_filter_worker(Clones, Opts, Results ++ [[Clone]]);
         NewClones when is_list(NewClones) ->
             main_filter_worker(Clones, Opts, Results ++ [NewClones]);
         true ->
             main_filter_worker(Clones, Opts, Results ++ [[]])
     end.

filter_conditions([Clone],_)->
    #clone_item{items = [Unit1, Unit2]} = Clone,
    eval_conditions({Unit1, Unit2}, one_element);
filter_conditions(Clones, Opts)->
    filter_and_trim(Clones, Opts, [], [], 0).

filter_and_trim([], Opts, Currents, Results, InvalidSeqLength)
  when (length(Currents) - InvalidSeqLength) == 1 ->
    MayBeResult = [hd(Currents)],
    case filter_conditions(MayBeResult, Opts) of
                true when Results /= []->
                    Results;
                true ->
                    true;
...
filter_and_trim([], _, Currents, Results, InvalidSeqLength) ->
...
filter_and_trim([ClI = #clone_item{items = [Unit1, Unit2], score=Score} | Clones],
    ...
    IsGood = Score >= DiffLimit andalso
                 not(eval_conditions({Unit1, Unit2}, multi_element)),
    case IsGood of
        true ->
            filter_and_trim(Clones, Opts, Currents++[ClI], Results, 0);
        false when InvalidSeqLength < MaxInvalidSeqLength andalso
                               Currents /= [] ->
            filter_and_trim(Clones, Opts, Currents++[ClI], Results, InvalidSeqLength + 1 );
        ...
        false when (length(Currents) - InvalidSeqLength) == 1 ->
            ...
            case filter_conditions(MayBeResult, Opts) of
                true ->
                    filter_and_trim(Clones, Opts, [], Results, 0 );
                false ->
                    filter_and_trim(Clones, Opts, [], Results ++ [MayBeResult],0)
            end;
        false ->
            filter_and_trim(Clones, Opts, [], Results, 0 )
    end.
...
eval_conditions(Params, one_element)->
    eval_conditions(Params, ?Metric:filter(filters_for_one_element_clones),false);
eval_conditions(Params, multi_element)->
    eval_conditions(Params, ?Metric:filter(filters_for_multi_element_clones),false).

eval_conditions(_, _,true)->
    true;
eval_conditions(_, [],Result)->
    Result;
eval_conditions(Par={ Unit1, Unit2}, [Filter | Filters], _)->
    ...
    eval_conditions(Par, Filters, Result).
%%% ============================================================================
