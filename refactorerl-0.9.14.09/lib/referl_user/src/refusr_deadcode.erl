...
%% @doc Start deadcode detection on all files.
start(Mode) when (Mode==print) or (Mode==term) ->
    ets:new(deadcodes,[bag,named_table]),
    ets:insert(deadcodes,{mode,Mode}),
    leaves(?Graph:root(),[]),
    DCList=ets:foldl(fun(E,Acc) -> [E|Acc] end,[],deadcodes),
    ets:delete(deadcodes),
    if Mode==term -> tl(DCList); Mode==print -> ok end;
...
start([_|_]=Filename,Mode)  when (Mode==print) or (Mode==term)->
...
check_rec(Node) ->
    ...
    if
        RecRefs==[] -> dc_message(unused_rec,Node,?Rec:name(Rec));
        true -> ok
    end,
    ...
    if
        ...
        not Exported ->
            Refs=?Query:exec(Funnode,?Query:all([?Fun:applications(),
                                                        ?Fun:implicits()])),
            if
                Refs==[] -> dc_message(unused_func,Node,?Fun:name(Funnode)),
                            false;
...
check_patterns([Pattern|Patterns],M,Prevs) ->
    ...
    case SimpGuard of
        {value,X} when X/=true ->
            dc_message(false_pattern_guard,P,lists:flatten(?Syn:tree_text(P))),
