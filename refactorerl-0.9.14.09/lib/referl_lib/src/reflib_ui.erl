...
-export([stop/1, reset/1, reset/2, clean/1, add/2, drop/2, backup/2, ls_backups/1,
         backup_info/2, current_posmode/1]).
...
%%% Dep. anal.s
-export([cyclic_fun_check_cycle/1, cyclic_fun_check_cycle/2,
         cyclic_fun_print_cycle/1, cyclic_fun_print_cycle/2,
         cyclic_fun_check_function/2,
         cyclic_fun_draw/1, cyclic_fun_draw/2,
         cyclic_fun_draw_cycles/1, cyclic_fun_draw_cycles/2,
         cyclic_fun_get_deps/1, cyclic_fun_get_deps/2]).

-export([cyclic_mod_check_module/2,
         cyclic_mod_print_cycle/1, cyclic_mod_print_cycle/2,
         cyclic_mod_check_cycle/1, cyclic_mod_check_cycle/2,
         cyclic_mod_draw/1, cyclic_mod_draw/2, 
         cyclic_mod_draw_cycles/1, cyclic_mod_draw_cycles/2,
         cyclic_mod_get_deps/1, cyclic_mod_get_deps/2]).
...
-export([refusr_fb_regexp_re/2,
         fb_relations_get_relations/3, fb_relations_is_relation/2,
         fb_relations_check_cycle/2, fb_relations_draw_cycles/2, 
         fb_relations_draw/2]).
...
-export([duplicated_code_const_var_diff/3, get_all_dupcode_result/1,
        get_dupcode_group/4,
        get_dupcode_result/3, get_algorithms/1, get_algorithm_data/2]).
...
%% @spec cyclic_fun_check_cycle(#msg_cb{}) -> {ok, {ok, no_cyclic_dependency} | {string(), list()}}
...
%% cycles, Cycle list}.
%% @see refusr_cyclic_fun:check_cycle/0
cyclic_fun_check_cycle(_MCB)->
    ?Dat(refusr_cyclic_fun:check_cycle()).

%% @spec cyclic_fun_check_cycle(#msg_cb{}, proplist()) -> {ok, {ok, no_cyclic_dependency} | {string(), list()}}
%% @doc Builds a directed graph, checks for cycles in function level. If
%% cycles are found then the following tuple is the result: {Number of
%% cycles, Cycle list}.
%% @see refusr_cyclic_fun:check_cycle/1
cyclic_fun_check_cycle(_MCB, ParList)->
    ?Dat(refusr_cyclic_fun:check_cycle(ParList)).

%% @spec cyclic_fun_print_cycle(#msg_cb{}) -> {ok, ok}
%% @doc Prints the result of check_cycle/0 to the standard output.
%% @see refusr_cyclic_fun:print_cycle/0
cyclic_fun_print_cycle(_MCB)->
    ?Dat(refusr_cyclic_fun:print_cycle()).

%% @spec cyclic_fun_print_cycle(#msg_cb{}, proplist() | node() | string()) -> {ok, ok}
%% @doc Prints the result of check_cycle/0 to the standard output.
%% @see refusr_cyclic_fun:print_cycle/1
cyclic_fun_print_cycle(_MCB, ParList)->
    ?Dat(refusr_cyclic_fun:print_cycle(ParList)).

%% @spec cyclic_fun_check_function(#msg_cb{},Function)-> {ok,Result}
%% Function = node() | string()
%% Result = {Bool | list() | {error, ErrorString, ErrorParam}}
%% Bool = true | false
%% @doc Creates a subgraph from the given function as a starting node
%% and checks for cycles. 
%% @see refusr_cyclic_fun:check_function/1
cyclic_fun_check_function(_MCB, Function)->
    ?Dat(refusr_cyclic_fun:check_function(Function)).

%% @spec cyclic_fun_draw(#msg_cb{})-> {ok, {ok, no_cyclic_dependency} | {integer(), list()}}
%% @doc Prints out the entire graph.
%% @see refusr_cyclic_fun:draw/0
cyclic_fun_draw(_MCB)->
    ?Dat(refusr_cyclic_fun:draw()).

%% @spec cyclic_fun_draw(#msg_cb{}, Options::proplists())-> {ok, {ok, no_cyclic_dependency} | 
%% {integer(), list()} | {error, ErrorString, ErrorParam} }
%% @doc Creates a subgraph drawing from the directed graph from a given function as a starting node.
%% @see refusr_cyclic_fun:draw/1
cyclic_fun_draw(_MCB, Options)->
    ?Dat(refusr_cyclic_fun:draw(Options)).

%% @spec cyclic_fun_draw_cycles(#msg_cb{})-> {ok, {ok, no_cyclic_dependency} | {integer(), list()}}
%% @doc Prints out a subgraph which contains the cycle(s).
%% @see refusr_cyclic_fun:draw_cycles/0
cyclic_fun_draw_cycles(_MCB)->
    ?Dat(refusr_cyclic_fun:draw_cycles()).

%% @spec cyclic_fun_draw_cycles(#msg_cb{}, PropList::proplists())-> {ok, {ok, no_cyclic_dependency} | 
%% {integer(), list()} | {error, bad_function_name} }
%% @doc Prints out a subgraph which contains the cycle(s) from the given node.
%% @see refusr_cyclic_fun:draw_cycles/1
cyclic_fun_draw_cycles(_MCB, Options)->
    ?Dat(refusr_cyclic_fun:draw_cycles(Options)).

cyclic_fun_get_deps(_MCB)->
    ?Dat(refusr_cyclic_fun:get_deps()).

cyclic_fun_get_deps(_MCB, StartingFunctions)->
    ?Dat(refusr_cyclic_fun:get_deps(StartingFunctions)).

%% @spec cyclic_mod_check_module(#msg_cb{}, Module)-> 
%% {ok, {integer(), list()} | false | {error, bad_module_name} | 
%% {error, ErrMsg::string(), ErrPars::list()}}
%% Module = node() | string()
%% @doc Prints out a subgraph which contains the cycle(s) from the given node.
%% @see refusr_cyclic_mod:check_module/1
cyclic_mod_check_module(_MCB, Module)->
    ?Dat(refusr_cyclic_mod:check_module(Module)).

%% @spec cyclic_mod_print_cycle(#msg_cb{}) -> {ok, ok}
%% @doc Prints the result of check_cycle/0 to the standard output.
%% @see refusr_cyclic_mod:print_cycle/0
cyclic_mod_print_cycle(_MCB)->
   ?Dat(refusr_cyclic_mod:print_cycle([])).

%% @spec cyclic_mod_print_cycle(#msg_cb{}, Module) -> {ok, ok}
%% Module = node() | string()
%% @doc Prints the result of check_cycle/1 to the standard output.
%% @see refusr_cyclic_mod:print_cycle/1
cyclic_mod_print_cycle(_MCB,FromNode)->
    ?Dat(refusr_cyclic_mod:print_cycle(FromNode)).

%% @spec cyclic_mod_check_cycle(#msg_cb{}) -> {ok, {ok, no_cyclic_dependency} | 
%% {integer(), list()} | {error, ErrMsg::string(), ErrPars::list()}}
%% @doc Checks for cycles in module level. 
%% @see refusr_cyclic_mod:check_cycle/0
cyclic_mod_check_cycle(_MCB)->
    ?Dat(refusr_cyclic_mod:check_cycle()).

%% @spec cyclic_mod_check_cycle(#msg_cb{}, proplist()) -> 
%% {ok, {ok, no_cyclic_dependency} | {integer(), list()} | 
%% {error, ErrMsg::string(), ErrPars::list()}}
%% @doc Checks for cycles in module level. 
%% @see refusr_cyclic_mod:check_cycle/1
cyclic_mod_check_cycle(_MCB, PropList)->
    ?Dat(refusr_cyclic_mod:check_cycle(PropList)).

%% @spec cyclic_mod_draw(#msg_cb{})-> {ok, {ok, no_cyclic_dependency} | {integer(), list()}}
%% @doc Prints out the entire graph.
cyclic_mod_draw(_MCB)->
    ?Dat(refusr_cyclic_mod:draw()).

%% @spec cyclic_mod_draw(#msg_cb{}, proplist())-> {ok, {ok, no_cyclic_dependency} | 
%% {integer(), list()} | {error, bad_function_name} }
%% @doc Creates a subgraph drawing from the directed graph from a given 
%% function as a starting node.
cyclic_mod_draw(_MCB, Proplist)->
    ?Dat(refusr_cyclic_mod:draw(Proplist)).

%% @spec cyclic_mod_draw_cycles(#msg_cb{})-> {ok, {ok, no_cyclic_dependency} | 
%%{integer(), list()}}
%% @doc Prints out a subgraph which contains the cycle(s).
cyclic_mod_draw_cycles(_MCB)->
    ?Dat(refusr_cyclic_mod:draw_cycles()).

%% @spec cyclic_mod_draw_cycles(#msg_cb{}, Proplist)-> {ok, {ok, no_cyclic_dependency} | 
%%{integer(), list()} | {error, bad_function_name}}
%% @doc Prints out a subgraph which contains the cycle(s) from the given node.
cyclic_mod_draw_cycles(_MCB, Proplist)->
    ?Dat(refusr_cyclic_mod:draw_cycles(Proplist)).

cyclic_mod_get_deps(_MCB)->
    ?Dat(refusr_cyclic_mod:get_deps()).

cyclic_mod_get_deps(_MCB, StartingModules)->
    ?Dat(refusr_cyclic_mod:get_deps(StartingModules)).
...
%% @spec fb_relations_get_relations(#msg_cb{}, ParList, Other) -> 
%% {ok, [Relation_Pairs] | {error, bad_argument}}
%%           ParList = [string()]
...
%% functionblock list.
fb_relations_get_relations(_MCB,ParList, Other)->
    ?Dat(refusr_fb_relations:get_relations(ParList, Other)).

%% @spec fb_relations_is_relation(#msg_cb{}, ParList)-> {ok, true | false | Error}
%%           ParList = tuple(Fb)
%%       Fb = string()
...
%% functionblocks.
fb_relations_is_relation(_MCB, ParList)->
    ?Dat(refusr_fb_relations:is_relation(ParList)).

%% @spec fb_relations_check_cycle(#msg_cb{}, ParList) -> {ok, 
...
%% functionblock list.
fb_relations_check_cycle(_MCB, ParList)->
    ?Dat(refusr_fb_relations: check_cycle(ParList)).

%% @spec fb_relations_draw(#msg_cb{}, PropList::proplist()) -> 
%%                   {ok, {ok, no_cyclic_dependency}
%%                      | {integer(), list()}
%%                      | {error, bad_function_name}}
%%
%% @doc Creates a subgraph drawing from the given functionblock list.
fb_relations_draw(_MCB, PropList)->
    ?Dat(refusr_fb_relations:draw(PropList)).

%% @spec fb_relations_draw_cycles(#msg_cb{}, PropList::proplist()) -> {ok, 
%%                   {ok, no_cyclic_dependency}
%%                      | {integer(), list()}
%%                      | {error, bad_function_name}}
%%
%% @doc From the given function block list creates a graph from the cycle list.
fb_relations_draw_cycles(_MCB, PropList)->
    ?Dat(refusr_fb_relations:draw_cycles(PropList)).
...
%% @spec get_dupcode_result(#msg_cb{}, atom(), atom()) -> {ok, any()}
%% @doc  Returns the Name stored dupcode result's
%% in Format format.
get_dupcode_result(_MCB, Name, Format)->
    try 
	    Result = refusr_clone_identifierl_lib:match_name_dets(Name, Format),
	    case Result of
		     {true, Res} -> ?Dat(Res);
		     _ -> ?Dat([])
	    end
...
do_depanal_and_transfer(_MCB, DepanalFunName, Arglist, Filename) ->
    DataDir = ?MISC:data_dir(),
    FullFilePath = filename:join([DataDir, Filename]),
    FullFilePathDOT = FullFilePath ++ ".dot",
    DepArglist = Arglist ++ [{dot, FullFilePathDOT}],
    case apply(?MODULE, DepanalFunName, [no_mcb, DepArglist]) of
