...
%% in parallel, transformations block until they are done.
do(MCB=#msg_cb{}, Mod, Args) when is_list(Args) ->
    Tab = ?RefChangeTab,
    dets:open_file(Tab, [{type, set}]),
    dets:delete_all_objects(Tab),
    dets:close(Tab),
    gen_server:call(?TRANSFORM_SERVER, {do, MCB, Mod, Args}, infinity).
...
-record(state, {
                ...
                next     :: integer(),
                % PID of currently running transformation or qinfos
                current = {qinfos, []} :: {qinfos, [#qinfo{}]} | #trinfo{},
...
handle_call({do, MCB, Mod, Args}, From, State) ->
    handle_do(MCB, From, Mod, Args, State);

handle_call({ask, Pid, PropLists}, _From, #state{current=#trinfo{pid=Pid}} = State) ->
...
handle_cast({finish, _Pid, Result, BadSmells}, #state{current=TrInfo=#trinfo{}, next=Next}) ->
    handle_tr_finished(Result, BadSmells, TrInfo),
    {noreply, #state{next=Next}}.

%% @private
handle_info({'EXIT', _Pid, Reason}, #state{current=TrInfo=#trinfo{}, next=Next} = _St) ->
...
%% @private
terminate(_R, #state{current=#trinfo{pid=Pid}}) when is_pid(Pid) ->
    exit(Pid, kill);
...
handle_do(_MCB, _From, _Mod, _Args, #state{current=#trinfo{pid=Pid}} = St) when is_pid(Pid) ->
    {reply, {error, ?LocalError(tr_running,[])}, St};
handle_do(MCB, From, Mod, Args, #state{current={qinfos, QInfos}, query_id=QId} = St) ->
    ...
    case BlockedTr of
        ...
        false ->
            CurrQId = QId + 1,
            TrPid = start_tr(MCB, Mod, Args, IsQuery),
            NewCurrent =
                case IsQuery of
                    false ->
                        init_trinfo(From, MCB, TrPid);
...
start_tr(MCB, Mod, Args, IsQuery) ->
    spawn_link(fun () -> do_transform(MCB, Mod, Args, IsQuery) end).
...
get_mcb_qt(#trinfo{mcb=MCB, question=QT}) ->
    {MCB, QT};
...
lookup_question(Num, #trinfo{question=QT, answer=AT, wait=WT}) ->
    case ets:lookup(QT, Num) of
        [{_, ID, Owner}] -> {ID, Owner, QT, AT, WT};
        []               -> error
    end;
...
lookup_answer(_Pid, #trinfo{answer=AT, wait=WT}) ->
    {AT, WT};
...
handle_pending(Pid, #state{current={qinfos, QInfos}}) ->
    #qinfo{question=QT} = lists:keyfind(Pid, #qinfo.pid, QInfos),
    pend(Pid, QT);
handle_pending(Pid, #state{current=#trinfo{question=QT}}) ->
    pend(Pid, QT).
...
handle_touch(Node, #state{current=TrInfo=#trinfo{save=Save}} = St) ->
    St#state{current=TrInfo#trinfo{save=?Syn:get_file(Node)++Save}}.

%% Remembers the files that have to be renamed.
%% The list `rename' is not unique until the point of saving.
%% Only transformations may attempt this operations, not queries.
handle_rename(Data,#state{current=TrInfo=#trinfo{rename=Rename}} = St) ->
    St#state{current=TrInfo#trinfo{rename=[Data|Rename]}}.
...
%% todo If the transformation has ended in an error, undo its changes.
...
do_transform(_MCB, Mod, Args, IsQuery) ->
    EndResult =
        try
