...
graph_control_panel()->
    ...
    TypeCell=#tablecell{body=[#label{text="Type:"},
                          #dropdown{id=graph_type,
                                    ...
                                    html_encode=true}],
                        class="graph_controll_tablecell"},
    SNodeModCell=#tablecell{body=[
        #label{text="Starting node (module):"},
    ...
    SNodeFunCell=#tablecell{body=[
        #label{text="Starting node (function):"},
        #table{rows=[#tablerow{cells=[
        #tablecell{body=[
...
get_parameters() ->
    ...
    case Gnode of
        ...
        mods ->
            GnodeM=get_text(starting_node_module),                
            StartingModname=try
                list_to_existing_atom(filename:basename(GnodeM,".erl"))
            catch _:_ -> 
                throw({error,"Error: The following module doesn't exist: "
                       ++ GnodeM}) 
            end,
            GnodeMTuple=case StartingModname of
                '' -> {gnodenone,none};
                _ -> {gnode,StartingModname}
            end,
            ExcludeMods=lists:map(fun(X) ->
                try
                    list_to_existing_atom(filename:basename(X,".erl"))
                catch _:_ -> 
                    throw({error,
                        "Error: The following module doesn't exist: "++ X}) end
                end,
                string:tokens(get_text(exclude_mod_list),"\n")),
            LeavesMods=lists:map(fun(X) -> 
                try 
                    list_to_existing_atom(filename:basename(X,".erl")) 
                catch _:_ -> 
                    throw({error,
                ...
                string:tokens(get_text(leaves_mod_list),"\n")),
                {[{level,list_to_existing_atom(Level)},
                        ...
                        {otp,OtpMod}],ok};
        funs -> 
            GnodeF=get_text(starting_node_function),
            StartingFunc=get_function(GnodeF),
            GnodeFTuple=case GnodeF of
                [] -> {gnodenone,none};
                _ -> {gnode,[StartingFunc]}
            end,
            ExcludeFuns=lists:map(fun get_function/1,
                string:tokens(get_text(exclude_func_list),"\n")),
            LeavesFuns=lists:map(fun get_function/1,
                string:tokens(get_text(leaves_func_list),"\n")),
            {[{level,list_to_existing_atom(Level)},
                        {type, list_to_existing_atom(Type)},
                        GnodeFTuple,
...
start_gen_fb_graph({Parameters, User})->
    ?NITRO_SERVICES:generate_fb_graph(Parameters, User).
