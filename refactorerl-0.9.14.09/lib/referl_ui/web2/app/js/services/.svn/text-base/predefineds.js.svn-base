...
/*    var sendAsDynfun = function(node) {
    ...
    var selectionDupcodeAlgorithms = ["matrix", "sw_metrics", "matrixfilter"];
    var selectionDupcodeTexts = {
        "matrix" : "Find Duplicates of Selection Using The Matrix Algorithm",
        "sw_metrics" : "Find Duplicates of Selection Using The Software Metrics Algorithm",
        "matrixfilter" : "Find Duplicates of Selection Using The Software Metrics With Syn. Filters Algorithm"
    };
            ...
            return predefineds.get(file, selection.cursorIndex).then(function(data) {
                ...
                if(selection.somethingSelected && (!data.error || data.error != "nofile") ) {
                    ...
                    for(var i in selectionDupcodeAlgorithms) {
                        ...
                        items.push({name: text, callback: callbackFor(file, selection, algorithm)});
                    }
                }
