...
function QueriesCtrl($scope, $rootScope, queries, locationSearch, $http, predefineds, smartLocation, jobs, jobType) {

	queries.setupNavigationIn($scope);

	$scope.historyType = "queries";
	...
	var run_again = function() {
		//console.log(queries.parameters());
...
// File tree code starts here!
	...
	$scope.extFilter = function(node) {
		if ($scope.options.onlyErlang) {
			if (node.folder) return true;
			return node.ext == "erl" || node.ext == "hrl";
		}
		return true;
	};
