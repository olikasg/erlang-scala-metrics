  ...
  return {
    ...
    link: function(scope, element, attrs) {
      ...
      scope.$watch(attrs.reD3Graph, function(graph) {
        ...
        // TODO FIXME:
        // send empty array instead of empty string...
        // There is a bug: at least the root function has a link
        if (graph.links === "") {
          graph.links = []
        }
        ...
        scope.$watchCollection('searchResults', function(results) {
          ...
          results.forEach(function(id) {
            d3.select('#' + id).classed('search-result', true);
          });
        });
        ...
        function dynamicUpdate() {
          ...
          circle = svg.append('g')
            ...
            .append('circle')
            .on('click', click)
            // TODO Do we need specific style based on type?
        ...
        function collapseNode(node) {
          ...
          // HÁTRA irány esetén:
          // I.)  Minden node-ot, amit előre találunk belepakoljuk a
          //      nodes-ba, mert azokat ki kell rajzolnunk!
          // II.) Minden olyan node-ot, amit hátrafelé találunk (azaz link.target = this)
          //      visible-re állítjuk, ha a távolságuk kisebb vagy egyenlő mint a maxDepth

          if (direction == 'both') {
            // Második része az algoritmusnak
            drawActualDirection(node, nodes, links, graphNodes, graphLinks);
          } else {
            // Első része az algoritmusnak
        ...
        function drawOppositeDirection(node, nodes, links, graphNodes, graphLinks) {
          // I.)
          // Bejárjuk a selectedNode-ból induló éleket!
          // 1.) linkek, amik ebből mutatnak!
          ...
          // 2.) Ezekről a linkekről elérhető összes node-ot bepakoljuk a nodes tömbbe,
          //     mert meg kell őket jeleníteni.
          ...
          // 3.) A most bepakolt node-okból is végre kell hajtani a bejárást, azaz
          //     a linkek bepakolását, szerintem ehhez elég a getLinksFromNode(node, graphLinks, 'both') függvény meghívása
        ...
        function tick() {
          path.attr('d', function(d) {
            ...
            return 'M' + (sourceX || 0) + ',' + (sourceY || 0) + 'L' + (targetX || 0) + ',' + (targetY || 0);
          });
