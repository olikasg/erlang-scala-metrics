...
k)*a.ratio/c):(a.stageX=i*(1-b)+j*b,a.stageY=o*(1-b)+k*b);a.dispatch("interpolate");if(1<=m)window.clearInterval(a.interpolationID),b=a.ratio,w?(a.ratio=h,a.stageX=j+(a.stageX-j)*a.ratio/b,a.stageY=k+(a.stageY-k)*a.ratio/b):(a.stageX=j,a.stageY=k),a.dispatch("stopinterpolate")}sigma.classes.Cascade.call(this);sigma.classes.EventDispatcher.call(this);var a=this;this.p={minRatio:0.5,maxRatio:32,marginRatio:1,zoomDelta:0.1,dragDelta:0.3,zoomMultiply:2,directZooming:!1,blockScroll:!0,inertia:1.1,mouseEnabled:!0};
...
function(g){b.graph.dropEdge(g);return f};this.pushGraph=function(g,d){g.nodes&&g.nodes.forEach(function(a){a.id&&(!d||!b.graph.nodesIndex[a.id])&&f.addNode(a.id,a)});g.edges&&g.edges.forEach(function(a){(validID=a.source&&a.target&&a.id)&&(!d||!b.graph.edgesIndex[a.id])&&f.addEdge(a.id,a.source,a.target,a)});return f};this.emptyGraph=function(){b.graph.empty();return f};this.getNodesCount=function(){return b.graph.nodes.length};this.getEdgesCount=function(){return b.graph.edges.length};this.iterNodes=
