...
-export([fb_relations/1, fb_relations_h/0]).

-export([fb_regexp/1, fb_regexp_h/0]).
...
draw_dep_fun(Options)    ->
    ...
    case proplists:get_value(type, Options) of
        all -> up_ui(ui({cyclic_fun_draw, Opts}));
        cycles -> up_ui(ui({cyclic_fun_draw_cycles, Opts}));
        undefined -> case proplists:get_value(gnode, Options) of
                undefined -> message("Error: bad argument keys given");
                _ ->  up_ui(ui({cyclic_fun_draw, Opts}))
                 end;
...
draw_dep_mod(Options) ->
    Opts=proplists:delete(type, Options),
    case proplists:get_value(type, Options) of
        all -> up_ui(ui({cyclic_mod_draw, Opts}));
        cycles -> up_ui(ui({cyclic_mod_draw_cycles, Opts}));
        undefined -> case proplists:get_value(gnode, Options) of
                undefined -> message("Error: bad argument keys given");
                _ -> up_ui(ui({cyclic_mod_draw, Opts}))
                 end;
        _ -> message("Error: bad type key given")
    end.
...
print_dep_fun(Options) ->
    Opts=proplists:delete(type, Options),
    case proplists:get_value(type, Options) of
        all -> up_ui(ui({cyclic_fun_check_cycle, Opts}));
        cycles -> case proplists:get_value(gnode, Options) of
                undefined -> up_ui(ui({cyclic_fun_print_cycle, Opts}));
                _ -> up_ui(ui({cyclic_fun_check_function, Opts}))
                 end;
        undefined -> case proplists:get_value(gnode, Options) of
                undefined -> message("Error: bad argument keys given");
                _ -> up_ui(ui({cyclic_fun_check_function, Opts}))
                 end;
        dep -> case proplists:get_value(gnode, Options) of
                   undefined -> up_ui(ui({cyclic_fun_get_deps}));
                   Node ->  up_ui(ui({cyclic_fun_get_deps, Node}))
               end;
        _ -> message("Error: bad type key given")
    end.
...
print_dep_mod(Options) ->
    Opts=proplists:delete(type, Options),
    case proplists:get_value(type, Options) of
        all -> up_ui(ui({cyclic_mod_check_cycle, Opts}));
        cycles -> case proplists:get_value(gnode, Options) of
                undefined -> up_ui(ui({cyclic_mod_check_module, Opts}));
                _ -> up_ui(ui({cyclic_mod_check_module, Opts}))
              end;
        undefined -> case proplists:get_value(gnode, Options) of
                undefined -> message("Error: bad argument keys given");
                _ ->  up_ui(ui({cyclic_mod_check_module, Opts}))
        ...
        dep -> case proplists:get_value(gnode, Options) of
                   undefined -> up_ui(ui({cyclic_mod_get_deps}));
                   Node ->  up_ui(ui({cyclic_mod_get_deps, Node}))
               end;
        _ -> message("Error: bad type key given")
    end.
...
draw_dep_h()->
    message("draw_dep(Options::proplists())
        ...
        - {gnode, Identifier | [Identifier]},
                 Identifier = node() | Module::atom() | Function::string()
        - {dot, Dot::string()}
        - {exception, Identifier | [Identifier]}
        - {leaves, Identifier | [Identifier]}
        - {otp, ExcludeOtp}, ExcludeOtp = true | false 
        ...
        - ri:draw_dep([{type, cycles}, {level, func}, {gnode, [\"ri:q/1\", \"ri:draw_dep/1\"]}]).
        - ri:draw_dep([{type, all}, {level, func}, {gnode, \"ri:q/1\"}, {otp, true}, {dot, \"/tmp/res.dot\"}]).
...
print_dep_h()->
    message("print_dep(Options::proplists())
        ...
        - ri:print_dep([{level, func}, {type, dep}, {gnode, [\"ri:print_dep_h/0\"]}]).
        - ri:print_dep([{type, cycles}, {level, func}, {gnode, [\"ri:print_dep_h/0\", \"ri:draw_dep_h/0\"]}]).
        - ri:print_dep([{type, all}, {level, func}, {gnode, \"ri:q/1\"}, {otp, true}]).
        - ri:print_dep([{type, all}, {level, mod}, {otp, true}, {exception, [ri, ris]}, {leaves, [reflib_ui]}]).").
...
fb_relations_h()->
    message("fb_relations(Options::proplists())-> GetRel | IsRel | Cycle | Error | ok\n"
           "Interface for functionblock examiner module.\n"
           "GetRel = [{Path::string(), Path::string()}]\n"
           "IsRel = true | false\n"
           "Cycle = {ok, no_cyclic_dependency} | {NoCycles::integer(), ListCycles::list()}\n"
           "Error = string() | tuple()\n"
           "Options = [Option]\n"
           "Option = {command, get_rel | is_rel | check_cycle | draw | draw_cycle} | \n"
           "{fb_list, [string()] | [{Basename::string(), [FunctionBlock::atom()]}]} | \n"
           "{other, Other::bool()}\n").
...
type_opt(get_rel, Options)->
    ...
    up_ui(ui({fb_relations_get_relations,
        convert_list(proplists:delete(other, Options)),
        Other}));

type_opt(is_rel, Options)->
    up_ui(ui({fb_relations_is_relation, Options}));
type_opt(check_cycle, Options)->
    up_ui(ui({fb_relations_check_cycle, Options}));
type_opt(draw, Options)->
    up_ui(ui({fb_relations_draw, Options}));
type_opt(draw_cycle, Options)->
    up_ui(ui({fb_relations_draw_cycles, Options}));
...
fb_regexp(Options)->
    up_ui(ui({refusr_fb_regexp_re, Options})).


fb_regexp_h()->
    message("fb_regexp(Options::proplists()) -> any()\n"
           "Interface for the function block regular expression filter.\n"
           "The Options are the following:\n"
           "{type, Type}\n"
           "Type = list | get_rel | cycle | draw\n"
           "{regexp, Value}\n"
...
"      output_path: string()\n\n" ++
 ...
 "Examples:\n" ++
"    DepOpts = [{level, mod}, {type, all}, {gnode, refusr_cyclic_mod}," ++
