...
-export([start/2, stop/0, execute_query/2, execute_graph/2, is_restricted/0,
...
dupcode_result_by_name(Name, Job) ->
    case make_ui_request({get_dupcode_result, Name, file_and_loc}) of
        {ok, Result} -> 
...
return_filelist(FileList) ->
    ...
    [[{path, Path},
      ...
      {folder, not NotDir}]
     || {NotDir, Path} <- Sorted,
        not starts_with(filename:basename(Path), ".")].
...
execute_graph_thread(Job) ->
    ...
    {Root, BaseOptions} = case Level of
        functionblock ->
            {array, SubjectsStructs} = proplists:get_value("subjects", Params),
            Subjects = lists:map(fun get_id/1, SubjectsStructs),
            {undefined, [{regexp, Subjects}, {type, draw}]};
        _ ->
            {{array, RootJson}, {array, ExcludeStructs}, {array, NoLeafStructs}} =
                case Level of
            ...
            {TheRoot, RootOptions} = case RootJson of
                [{struct, _}|_] -> RootIds = lists:map(fun get_id_as_atom/1,RootJson),
                               RootOpts = [{gnode, RootIds}],
                               {RootIds, RootOpts};
                _           -> {undefined, []}
            end,
            RestOptions =
                [{exception, lists:map(fun get_id_as_atom/1, ExcludeStructs)},
                 {leaves, lists:map(fun get_id_as_atom/1, NoLeafStructs)},
                 {otp, proplists:get_value("excludeOtp", Params)}],
            {TheRoot, RootOptions ++ RestOptions}
    end,

    BaseName = "web2_" ++ Owner ++ "_" ++ integer_to_list(Id),
    Result = case Format of
        text ->
            RequestName = case Level of
                              module   -> cyclic_mod_get_deps;
                              function -> cyclic_fun_get_deps
                          end,
            Request = case Root of
        ...
        collapsible ->
            case draw_graph(Level, Type, BaseOptions ++ [{output, term}]) of
                {ok, Term} ->
...
execute_duplicates_by_selection(Job) ->
    ...
    Req = {clone_identifierl_by_pos, Algorithm, File,
            LineCol(From), LineCol(To), linecol, file_and_loc},
...
execute_duplicates_thread(Job) ->
    ...
    Options = [{format, file_and_loc}, {postype, linecol}] ++
...
draw_graph(Level, Type, Options, DotFile) ->
    FullOptions = Options ++ [{dot, DotFile}],
    draw_graph(Level, Type, FullOptions).

draw_graph(Level, Type, Options) ->
    RequestName = draw_graph_request_name(Level, Type),
    make_ui_request({RequestName, Options}).

draw_graph_request_name(module,   all)    -> cyclic_mod_draw;
draw_graph_request_name(module,   cycles) -> cyclic_mod_draw_cycles;
draw_graph_request_name(function, all)    -> cyclic_fun_draw;
draw_graph_request_name(function, cycles) -> cyclic_fun_draw_cycles;
draw_graph_request_name(functionblock, _) -> refusr_fb_regexp_re.
