...
start([S | _]=UserName) ->
    NodeName = node(),
    if 
       is_integer(S) -> User = UserName;
       true -> User = atom_to_list(S)
    end,
    ...
    Binary = "lib/referl_ui/qt/bin/RefErlQt",
...
%Dependency graph
handle_gui_request({draw_svg, Level, Type, Path, Options}, GuiPid) ->
    GuiPid ! {draw_svg, draw_svg(Path, Level, Type, Options)};

handle_gui_request({print_graph, Level, Type, _Path, Options}, GuiPid) ->
    GuiPid ! {print_graph, print_graph(Level, Type, Options)};
...
%% @spec dependency_analysis(atom(), atom(), proplist(), boolean() ) -> 
%% string() | {error, string()}
dependency_analysis(Level, Type, Opts, ComplexLabel) ->
    DotPath = 
        filename:join( 
            ?MISC:data_dir(), 
            "latest_dependency_res" ++ ?MISC:graph_based_postfix()
        ),
    CL = if ComplexLabel -> [{label_mode, complex_label}];
            true   -> []
    end,
    Options = Opts ++ [{dot, DotPath}] ++ CL,
    R =
        case {Level, Type} of 
            {mod, all} -> 
                ui({cyclic_mod_draw, Options}); 
            {mod, cycles} -> 
                ui({cyclic_mod_draw_cycles, Options}); 
            {func, all} -> 
                ui({cyclic_fun_draw, Options}); 
            {func, cycles} -> 
                ui({cyclic_fun_draw_cycles, Options}); 
            {modgroup, all} -> 
                ui({refusr_fb_regexp_re, Options ++ [{type, draw}]}); 
            {modgroup, cycles} -> 
                ui({refusr_fb_regexp_re, Options ++ [{type, draw}]}) 
        end,
    case R of
...
%% @spec draw_svg(string(), atom(), atom(), porplist()) -> 
%% string() | {error, string()}
draw_svg(Path, Level, Type, Opts) ->
    try
        R = dependency_analysis(Level, Type, Opts, false),
        case R of
            {error, Error} -> ?ERROR(Error);
            DotPath -> 
...
%% @spec print_graph(atom(), atom(), proplist()) -> string() | {error, string()}
print_graph(Level, Type, Opts) ->
    try
        R = dependency_analysis(Level, Type, Opts, true),
    case R of
        {error, Error} -> ?ERROR(Error);
        DotPath -> 
            case get_file_if_needed(DotPath) of
...
pids_to_string(InvElem) ->
    ...
    if
        is_atom(Parent) ->
            P = atom_to_list(Parent);
        true ->
            P = pid_to_list(Parent)
    end,
...
strings_to_pids(InvElem) ->
    {Id, Parent, Node, Children} = InvElem,
    if 
        Parent == "no_parent" -> P = list_to_atom(Parent);
        true -> P = list_to_pid(Parent)
    end,
...
run_previous_dupcode_analysis(Name) ->
    ...
    case lists:member(Name, Names) of
        ...
        true -> 
            [Result] = lists:filter(fun(Res) -> [[N] | _ ] = Res, N == Name end, Results),
