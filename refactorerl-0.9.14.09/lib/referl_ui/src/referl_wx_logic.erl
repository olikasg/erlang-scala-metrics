...
%% @spec run_query_or_skel(string(), string(), {string(), integer()}) -> 
%%       ok | wx_error()
...
node_file_pos(Node) ->
    case node_type(Node) of
        module  -> Pos = no_pos,
                File = mod_file(Node),
                {File, Pos};
        fundef -> Pos = fundef_pos(Node),
                File = fundef_file(Node),
                {File, Pos};
        _    -> ?wxErr("Unknown node")
    end.
...
dep_analysis(Level, Type, FilePath, Opts, ClMode) ->
    CL = if ClMode -> [{label_mode, complex_label}];
            true   -> []
    end,
    Options = CL ++ [{dot, FilePath}] ++ Opts,
    R =
    case {Level, Type} of
        {func,all}       -> ui({cyclic_fun_draw, Options});
        {mod,all}        -> ui({cyclic_mod_draw, Options});
        {modgroup,all}   -> ui({refusr_fb_regexp_re, Options ++ [{type, draw}]});
        {func,cycle}     -> ui({cyclic_fun_draw_cycles, Options});
        {mod,cycle}      -> ui({cyclic_mod_draw_cycles, Options});
        {modgroup,cycle} -> ui({refusr_fb_regexp_re, Options ++ [{type, draw}]})
    end,
    case R of
        {error, Error} -> ?wxError(Error);
        _               -> FilePath
...
%%       term() | wx_error()
print_dep_graph(Level, _Type, Options) ->
...
%print_dep_graph(_Level, _Type, _Options) ->
...
get_dupcode_result(Name) ->
    case ui({get_dupcode_result, Name, file_and_loc}) of
        {error, _ } -> {Name, undefined};
        Res         -> {Name, proplists:get_value(detected_clones, Res)}
    end.

get_dupcode_by_pos(ModulePath, Start, End) ->
    case ui({clone_identifierl_by_pos, ModulePath, Start, End, scalar, file_and_loc}) of
        {error, E } -> ?wxError(E);
...
parse_graphnode_line(E) ->
    try
        ...
        CLabel = get_clabel(lists:nthtail(6,L)),
        NW = list_to_num(W), 
        NH = list_to_num(H),
...
parse_graphedge_line(E,Nodes) ->
    try 
        ...
        ArrInfo = arrow_info(CX,CY,R,lists:last(Points)),
        StartPoint = get_intersection_point(CX2,CY2,R2,hd(Points)),
        Points2 = [StartPoint|Points],
        {{From, To}, 
