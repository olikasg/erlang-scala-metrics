...
init_browser(Panel) ->
    ...
    wxMenu:append(PopupMenu, ?DUPSEARCH_ID, "Search duplicates in selection..."),
...
%% Right click
handle_event(#wx{event=#wxContextMenu{}, obj=Obj}, 
             State = #state{ parent=Panel, menu=PopupMenu }) ->
    wxWindow:popupMenu(Panel, PopupMenu),
    {noreply, State#state{}};

handle_event(#wx{event=#wxCommand{ type=command_menu_selected }, obj=Obj}, 
             State = #state{ textctrl=TextCtrl, menu=PopupMenu,
                             handler=DupHandler }) ->
    StartPos = ?stc:getSelectionStart(TextCtrl),
    EndPos = ?stc:getSelectionEnd(TextCtrl),
    ModulePath = State#state.fpath,
    if StartPos =< EndPos ->
               ask_for_dupsearch(DupHandler, ModulePath, StartPos, EndPos);
       true -> ok
    end,
    {noreply, State#state{}};
...
ask_for_dupsearch(DupHandler, ModuleName, StartPos, EndPos) ->
    DupHandler ! {request_for_dupcode_search, ModuleName, StartPos, EndPos}.
