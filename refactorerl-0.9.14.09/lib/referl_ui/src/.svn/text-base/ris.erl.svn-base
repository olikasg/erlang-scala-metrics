...
%%         - ris:print_dependencies([{level, mod}, {type, dep}]).
%%         - ris:print_dependencies([{level, func}, {type, dep}, {gnode, [\"ri:print_dep_h/0\"]}]).
%%         - ris:print_dependencies([{type, cycles}, {level, func}, {gnode, [\"ri:print_dep_h/0\", \"ri:draw_dep_h/0\"]}]).
%%         - ris:print_dependencies([{type, all}, {level, func}, {gnode, \"ri:q/1\"}, {otp, true}]).
%%         - ris:print_dependencies([{type, all}, {level, mod}, {otp, true}, {exception, [ri, ris]}, {leaves, [reflib_ui]}]).
%% @end
print_dependencies([]) -> {error, "Error: no option list given"};
print_dependencies(Options) ->
    case proplists:get_value(level, Options) of
        ...
        _ -> {error, "unknown dependency level given."}
    end.

print_dep_fun([]) -> {error, "Error: no type or gnode given"};
print_dep_fun(Options) ->
    Opts=proplists:delete(type, Options),
    case proplists:get_value(type, Options) of
        all -> ui({cyclic_fun_check_cycle, Opts});
        cycles -> case proplists:get_value(gnode, Options) of
                undefined -> ui({cyclic_fun_print_cycle, Opts});
                _ -> ui({cyclic_fun_check_function, Opts})
                 end;
        undefined -> case proplists:get_value(gnode, Options) of
                undefined -> {error, "Error: bad argument keys given"};
                _ -> ui({cyclic_fun_check_function, Opts})
                 end;
        dep -> case proplists:get_value(gnode, Options) of
                   undefined -> ui({cyclic_fun_get_deps});
                   Node ->  ui({cyclic_fun_get_deps, Node})
               end;
        _ -> {error, "Error: bad type key given"}
    end.

print_dep_mod([]) -> {error, "Error: no type or gnode given"};
print_dep_mod(Options) ->
    Opts=proplists:delete(type, Options),
    case proplists:get_value(type, Options) of
        all -> ui({cyclic_mod_check_cycle, Opts});
        cycles -> case proplists:get_value(gnode, Options) of
                undefined -> ui({cyclic_mod_check_module, Opts});
                _ -> ui({cyclic_mod_check_module, Opts})
              end;
        undefined -> case proplists:get_value(gnode, Options) of
                undefined -> {error, "Error: bad argument keys given"};
                _ ->  ui({cyclic_mod_check_module, Opts})
        ...
        dep -> case proplists:get_value(gnode, Options) of
                   undefined -> ui({cyclic_mod_get_deps});
                   Node ->  ui({cyclic_mod_get_deps, Node})
               end;
        _ -> {error, "Error: bad type key given"}
    end.
...
show_dupcode(Name, Format)
    ...
    case ui({get_dupcode_result, Name, Format}) of 
        {ok,[]} ->
