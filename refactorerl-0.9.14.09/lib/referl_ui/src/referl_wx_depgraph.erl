...
create_node_button(Sc, Parent, CX, CY, Label, NodeRep, W, H, R, Rank,MinRank,
                   DrwTools) ->
    if Rank >= MinRank ->
       draw_node_button(Parent, ?F(CX*Sc), ?F(CY*Sc), {CX,CY}, Label, 
                        NodeRep, ?F(W*Sc), ?F(H*Sc), ?F(R*Sc), DrwTools);
     true -> 
       ok
    end.
...
draw_node_button_lin(Parent, CX, CY, W, H, R, DrwTools) -> 
    {Brush, Pens} = DrwTools,
...
add_option_widgets(Panel,OptionsSz,?modgroupLvl) ->
    ...
    MGList = add_list_widget(Panel,OptionsSz,RInp,
                              "Modulegroups", fun handle_add_correct_regexp/2),  
...
add_funmod_ws(Panel, OptionsSz, L) ->
    {StartLab, StartInp} 
        = add_input(Panel, OptionsSz, "Start " ++ L),
    wxSizer:addSpacer(OptionsSz,10),
    ChkBox = add_otp_exclude_chkbox(Panel,OptionsSz),
    wxSizer:addSpacer(OptionsSz,10),
    ExcluList = add_list_widget(Panel,OptionsSz,StartInp,"Excluded "++L++"s"),
    wxSizer:addSpacer(OptionsSz,10),
    LvsList = add_list_widget(Panel,OptionsSz,StartInp,
                              "Exclude leaves of these "++L++"s"),
    {StartInp, StartLab, ExcluList, LvsList, ChkBox}.
...
add_list_widget(Parent,Sz,Input,L) ->
    add_list_widget(Parent,Sz,Input,L,fun handle_add/2).
add_list_widget(Parent,Sz,Input,L,AddHandler) ->
    Label = wxStaticText:new(Parent, ?wxID_ANY, L),
    ListW = wxListBox:new(Parent, ?wxID_ANY, [{size, {200, 100}}]),
    S = {size, {100, 22}},
...
gather_options(Ws) ->
    case Ws of
        {_,_,ModGrList} -> [{regexp, list_from_listwidget(ModGrList)}];
        {StartInp, _, ExcList, LvsList, OtpBox} ->
                           [input_to_gnode(StartInp),
                            {exception,list_from_listwidget(ExcList)},
                            {leaves,list_from_listwidget(LvsList)},
                            {otp, otpbox(OtpBox)}];
        _ -> []
    end.
