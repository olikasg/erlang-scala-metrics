...
void RefErlModel::DrawGraph(const QString &path,
    ...
    const QString &start_node,
    const QStringList &excluded_nodes,
    const QStringList &excluded_leaves, const bool &svg) const
{
    ETERM *draw_graph_msg[5], *tuple_msg;
    if(svg) {
    ...
    switch(level) {
        case Module: {
            draw_graph_msg[1] = erl_mk_atom("mod");
            break;
        }
        case ModuleGroup: {
            draw_graph_msg[1] = erl_mk_atom("modgroup");
            break;
        }
        case Function: {
            draw_graph_msg[1] = erl_mk_atom("func");
            break;
        }
    }

    draw_graph_msg[2] = type == All ? erl_mk_atom("all") : erl_mk_atom("cycles");
    draw_graph_msg[3] = erl_mk_estring(path.toStdString().c_str(), path.length());
    ETERM *option_list = erl_mk_empty_list();
    //Start node
    if(start_node != "") {
        ETERM *start_node_msg[2];
        start_node_msg[0] = erl_mk_atom("gnode");
        start_node_msg[1] = erl_mk_estring( start_node.toStdString().c_str(),
                                            start_node.length() );
        option_list = erl_cons( erl_mk_tuple(start_node_msg, 2), option_list);
    }
    //Exclude otp
    ETERM *exclude_otp_msg[2];
    exclude_otp_msg[0] = erl_mk_atom("otp");
    ...
    excluded_msg[0] = erl_mk_atom("exception");
    ...
    excluded_leaves_msg[0] = erl_mk_atom("leaves");
    ...
    draw_graph_msg[4] = option_list;
    tuple_msg = erl_mk_tuple(draw_graph_msg, 5);
    ref_erl_connect_->SendMessage(tuple_msg);
    erl_free_compound(tuple_msg);
