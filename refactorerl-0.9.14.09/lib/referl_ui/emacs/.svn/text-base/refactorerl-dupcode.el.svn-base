...
(defun refactorerl-show-dupcode ()
  ...
  (refac-send/callbacks ('get_dupcode_result code-to-show :file_and_loc)
	 (:reply (ok result)
	 	(if (equal result "")
...
(defun refactorerl-dupcode-selected ()
  (interactive)
  ...
  (refac-send/callbacks ('clone_identifierl_by_pos modulename (region-beginning) (region-end) :linecol :file_and_loc)
	 (:reply (ok result)
	 	(if (equal result "")
