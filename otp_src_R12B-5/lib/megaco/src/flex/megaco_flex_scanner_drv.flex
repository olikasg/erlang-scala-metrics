...
static void mfs_load_property_groups();
  ...
  */
  ...
  if ((mfs_term_spec_index + size) >= mfs_term_spec_size) {
    ...
    DBG( ("mfs_ensure_term_spec -> "
	  ...
	  (unsigned int) mfs_term_spec, mfs_term_spec_size) );
...
static void mfs_octet_load_token(ErlDrvTermData TokenTag, int is_empty)
{
  ...
  if (is_empty) {
    ...
    ASSIGN_TERM_SPEC(ERL_DRV_NIL);         // End of list

  } else {

    mfs_load_property_groups();
...
static void mfs_lower_load_token(ErlDrvTermData TokenTag, int is_empty)
{
  ...
  if (is_empty) {
    ...
    ASSIGN_TERM_SPEC((ErlDrvTermData)"");
    ASSIGN_TERM_SPEC(0);
...
static void mfs_load_property_groups()
{
  ...
  mfs_ensure_term_spec(10);  /* Just in case... */
  ...
  while (i <= yyleng) {
    ...
    if ((yytext[i] != SP)   && 
        ...
        if (yytext[i] == '=') {
          ...
          if (0 == strncmp("v", &yytext[nameStart], nameLen)) {
            ...
            if (numGroups > 0) {
              ...
              mfs_ensure_term_spec(3);
              ASSIGN_TERM_SPEC(ERL_DRV_NIL);                  // End of list
              ASSIGN_TERM_SPEC(ERL_DRV_LIST);                 // List
              ASSIGN_TERM_SPEC((ErlDrvTermData) numPps + 1);  // Length of (group) list
          ...
          /* Name */
          name = ALLOC(nameLen + 1); // Make room for the NULL termination
          ...
          if (valueLen == 0) {
            ...
            if (0 >= sprintf(mfs_error_msg, "%s %s %s", 
                             PG_ERR_PRE, PG_ERR1, name)) {
              mfs_fatal_error(PG_ERR1);
            }
            mfs_error = TRUE;
            ...
            mfs_ensure_term_spec(15);       // 2 + 3 + 6 + 2 + 2
  
            DBG( ("mfs_load_property_groups -> "
                  "insert PropertyParm record name\n") );
            ASSIGN_TERM_SPEC(ERL_DRV_ATOM);
            ASSIGN_TERM_SPEC((ErlDrvTermData) mfs_PropertyParm);
  	  
            /* Name */
            DBG( ("mfs_load_property_groups -> insert name field\n") );
            ASSIGN_TERM_SPEC(ERL_DRV_STRING);
            ASSIGN_TERM_SPEC((ErlDrvTermData) name);
            ASSIGN_TERM_SPEC((ErlDrvTermData) nameLen);
            ...
            value = ALLOC(valueLen);
            strncpy(value, &yytext[valueStart], valueLen);
            ASSIGN_TERM_SPEC(ERL_DRV_STRING);
            ASSIGN_TERM_SPEC((ErlDrvTermData) value);
            ASSIGN_TERM_SPEC((ErlDrvTermData) valueLen);
            ASSIGN_TERM_SPEC(ERL_DRV_NIL);             // End of value list
            ASSIGN_TERM_SPEC(ERL_DRV_LIST); 
            ASSIGN_TERM_SPEC((ErlDrvTermData) 1 + 1);  // Length of (group) list
  	  
            /* extraInfo - never used */
            DBG( ("mfs_load_property_groups -> insert the extraInfo field\n") );
            ASSIGN_TERM_SPEC(ERL_DRV_ATOM);
            ASSIGN_TERM_SPEC((ErlDrvTermData) mfs_asn1_NOVALUE);
  
            DBG( ("mfs_load_property_groups -> "
                  "terminate PropertyParm tuple\n") );
            ASSIGN_TERM_SPEC(ERL_DRV_TUPLE);
            ASSIGN_TERM_SPEC((ErlDrvTermData) 4);
            break;
      ...
      if (nameLen == 0) {
        ...
        if (0 >= sprintf(mfs_error_msg, "%s %s (name start at %d)", 
                         PG_ERR_PRE, PG_ERR2, nameStart)) {
          mfs_fatal_error(PG_ERR2);
        }
  ...
  mfs_ensure_term_spec(4); // 2 + 2 just in case
  ...
  ASSIGN_TERM_SPEC(ERL_DRV_NIL);                    // End of list
  ASSIGN_TERM_SPEC(ERL_DRV_LIST);
  ASSIGN_TERM_SPEC((ErlDrvTermData) numGroups + 1); // Length of (groups) list
